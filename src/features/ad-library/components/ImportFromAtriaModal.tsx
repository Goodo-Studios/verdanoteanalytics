...
export function ImportFromAtriaModal({ isOpen, onClose }: ImportFromAtriaModalProps) {
  ...
  const bookmarkletCode = `javascript:void(async function(){try{if(!location.hostname.includes('tryatria.com')&&!location.hostname.includes('atria')){alert('Please navigate to tryatria.com first');return}var delay=function(ms){return new Promise(function(r){setTimeout(r,ms)})};var unique=function(arr){return arr.filter(Boolean).filter(function(v,i,self){return self.indexOf(v)===i})};var videoRe=/\.(mp4|webm|mov|m3u8|avi)(\?|$)/i;var normalizeUrl=function(url){if(!url)return null;try{return new URL(url,location.href).href}catch(e){return null}};var extractBg=function(el){var bg=getComputedStyle(el).backgroundImage||'';var m=bg.match(/url\(["']?(.*?)["']?\)/);return m&&m[1]?normalizeUrl(m[1]):null};var flatten=function(input){if(!input)return[];if(Array.isArray(input))return input.flatMap(flatten);if(typeof input==='object'){var values=Object.values(input);var maybeMedia=[];if(typeof input.video_url==='string')maybeMedia.push(input.video_url);if(typeof input.thumbnail_url==='string')maybeMedia.push(input.thumbnail_url);if(typeof input.image_url==='string')maybeMedia.push(input.image_url);if(Array.isArray(input.media_urls))maybeMedia=maybeMedia.concat(input.media_urls);if(Array.isArray(input.mediaUrls))maybeMedia=maybeMedia.concat(input.mediaUrls);if(typeof input.source_url==='string'||typeof input.url==='string'){return [{source_url:input.source_url||input.url,landing_page_url:input.landing_page_url||input.destination_url||null,advertiser_name:input.advertiser_name||input.brand_name||input.advertiser||'Unknown',body_text:input.body_text||input.body||input.text||null,platform:input.platform||'facebook',ad_format:input.ad_format||input.format||(maybeMedia.some(function(u){return typeof u==='string'&&videoRe.test(u)})?'video':null),thumbnail_url:input.thumbnail_url||input.image_url||null,media_urls:unique(maybeMedia.map(normalizeUrl).filter(Boolean)),raw_data:input}]}return values.flatMap(flatten)}return[]};var overlay=document.createElement('div');overlay.style.cssText='position:fixed;top:20px;right:20px;z-index:99999;background:#18181b;color:white;padding:16px 24px;border-radius:12px;font-family:system-ui;font-size:14px;box-shadow:0 8px 32px rgba(0,0,0,.3);max-width:320px';overlay.innerHTML='Scanning Atria page…';document.body.appendChild(overlay);var networkAds=[];if(!window.__atriaFetchPatched){window.__atriaFetchPatched=true;var originalFetch=window.fetch;window.fetch=function(){return originalFetch.apply(this,arguments).then(async function(resp){try{var clone=resp.clone();var text=await clone.text();if((text&&text[0]==='{')||(text&&text[0]==='[')){var data=JSON.parse(text);var serialized=JSON.stringify(data);if(/video_url|media_urls|thumbnail|advertiser|headline|body_text/i.test(serialized)){networkAds.push(data)}}}catch(e){}return resp})}}var lastHeight=-1;for(var pass=0;pass<10;pass++){window.scrollTo({top:document.body.scrollHeight,behavior:'auto'});await delay(900);overlay.innerHTML='Scanning Atria page… pass '+(pass+1)+'/10';if(document.body.scrollHeight===lastHeight)break;lastHeight=document.body.scrollHeight}window.scrollTo({top:0,behavior:'auto'});await delay(250);var ads=[];var seen={};var cards=document.querySelectorAll('[class*="card"],[class*="Card"],[class*="ad-item"],[class*="AdCard"],[class*="swipe"],article,div[class*="grid"]>div');cards.forEach(function(c,idx){if(c.offsetHeight<50)return;var text=(c.innerText||'').trim();var images=Array.from(c.querySelectorAll('img')).map(function(img){return normalizeUrl(img.currentSrc||img.src)}).filter(Boolean);var videos=Array.from(c.querySelectorAll('video')).reduce(function(acc,video){var direct=normalizeUrl(video.currentSrc||video.src);if(direct)acc.push(direct);Array.from(video.querySelectorAll('source')).forEach(function(s){var src=normalizeUrl(s.src);if(src)acc.push(src)});return acc},[]).filter(Boolean);var media=unique(videos.concat(images));var thumb=images[0]||extractBg(c)||null;var link=c.querySelector('a[href*="facebook.com"],a[href*="tiktok.com"],a[href*="ads/library"],a[href*="instagram.com"]');var lpLink=c.querySelector('a[href^="http"]');var nameEl=c.querySelector('h1,h2,h3,h4,[class*="brand"],[class*="name"],[class*="advertiser"]');var name=nameEl?nameEl.textContent.trim():'';var lines=text.split('\\n').map(function(l){return l.trim()}).filter(Boolean);if(!name&&lines.length>0)name=lines[0].substring(0,80);if(!text&&media.length===0)return;var sourceUrl=link?link.href:(lpLink?lpLink.href:'https://tryatria.com/saved/'+idx+'-'+Date.now()+'-'+Math.random().toString(36).substring(2,8));if(seen[sourceUrl])return;seen[sourceUrl]=true;ads.push({source_url:sourceUrl,landing_page_url:lpLink&&lpLink.href!==sourceUrl?lpLink.href:null,advertiser_name:name||'Unknown',body_text:text.substring(0,4000)||null,platform:/tiktok/i.test(sourceUrl)?'tiktok':'facebook',ad_format:videos.length?'video':media.length>1?'carousel':thumb?'image':null,thumbnail_url:thumb,media_urls:media,raw_data:{captured_from:'atria-bookmarklet',page_url:location.href,card_index:idx,text:text,video_urls:videos,image_urls:images}})});ads=ads.concat(flatten(networkAds).filter(function(ad){return ad.source_url&&!seen[ad.source_url]&&(seen[ad.source_url]=true)}));if(!ads.length){overlay.innerHTML='No ads found. Make sure you are on your saved ads view and have scrolled to load all ads.';setTimeout(function(){overlay.remove()},5000);return}overlay.innerHTML='Found '+ads.length+' ads. Sending to AdVault…';var token=prompt('Paste your AdVault auth token (from the Import modal copy button):');if(!token){overlay.remove();return}fetch('${import.meta.env.VITE_SUPABASE_URL}/functions/v1/import-ads',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({ads:ads})}).then(function(r){return r.json()}).then(function(data){overlay.innerHTML='<b>Done!</b> Imported '+(data.imported||0)+' ads.'+(data.skipped_duplicates?' ('+data.skipped_duplicates+' duplicates skipped)':'');setTimeout(function(){overlay.remove()},7000)}).catch(function(e){overlay.innerHTML='Import failed: '+e.message;setTimeout(function(){overlay.remove()},7000)})}catch(err){alert('Atria bookmarklet failed: '+(err&&err.message?err.message:err))}})()`;
