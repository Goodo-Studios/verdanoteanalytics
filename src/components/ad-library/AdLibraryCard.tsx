import { SavedAd, useDeleteSavedAd } from "@/hooks/useAdLibrary";
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] bg-muted flex items-center justify-center overflow-hidden">
        {ad.thumbnail_url ? (
          <img
            ...
            alt={ad.headline || ad.brand_name || "Ad"}
        ...
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
              {ad.landing_page_url && (
                <DropdownMenuItem onClick={() => window.open(ad.landing_page_url!, "_blank")}>
                  <ExternalLink className="h-3.5 w-3.5 mr-2" /> Visit landing page
                </DropdownMenuItem>
              )}
              <DropdownMenuItem
      ...
      <div className="flex flex-col gap-1.5 p-3">
        {ad.brand_name && (
          <p className="font-label text-[11px] uppercase tracking-wider text-sage truncate">{ad.brand_name}</p>
        )}
        {ad.headline && (
        ...
        {ad.tags && ad.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1">
            ...
            {ad.tags.length > 3 && (
              <span className="text-[10px] text-sage">+{ad.tags.length - 3}</span>
            )}
          </div>
        )}
      </div>
