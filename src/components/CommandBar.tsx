...
import {
  Search, FileText, Users, FlaskConical, LayoutGrid,
  FileEdit, RefreshCw, CalendarDays, Link2, ArrowRightLeft,
  Clock, Zap, Settings, BarChart3, Download, Heart,
...
export function CommandBar() {
  ...
  const slashCommands = useMemo((): SlashCommand[] => {
    ...
    if (currentAccountId) {
      cmds.push({
        command: "/sync",
        label: "Trigger Sync",
        description: "Sync current account with Meta",
    ...
    cmds.push({
      command: "/retro",
      label: "Weekly Retro",
      description: "Open weekly retro modal",
      icon: <CalendarDays className="h-4 w-4" />,
      execute: () => { setOpen(false); navigate("/settings"); },
    });
    cmds.push({
      command: "/brief",
      label: "New Brief",
      ...
      execute: () => { setOpen(false); navigate("/briefs"); },
    });
    cmds.push({
      command: "/report",
      label: "New Report",
      ...
      execute: () => { setOpen(false); navigate("/reports"); },
    });
    cmds.push({
      command: "/compare",
      label: "Compare View",
      description: "Open creative comparison",
      icon: <BarChart3 className="h-4 w-4" />,
      execute: () => { setOpen(false); navigate("/compare"); },
    });
    cmds.push({
      command: "/export",
      label: "Export Center",
      description: "Open data export tools",
    ...
    cmds.push({
      ...
      description: "Show client health score",
    ...
    cmds.push({
      ...
      description: "Usage: /grade A — filter creatives by grade",
      icon: <Zap className="h-4 w-4" />,
      execute: (args) => {
        const grade = args.trim().toUpperCase();
        if (["A", "B", "C", "D", "F"].includes(grade)) {
          setOpen(false);
          navigate(`/creatives?grade=${grade}`);
        } else {
          toast.error("Invalid grade. Use A, B, C, D, or F");
        }
      },
    });
    cmds.push({
      command: "/roas",
      label: "Filter by ROAS",
      description: "Usage: /roas >2.0 or /roas <1.0",
      icon: <BarChart3 className="h-4 w-4" />,
      execute: (args) => {
        const match = args.trim().match(/^([><])(\d+\.?\d*)$/);
        if (match) {
          setOpen(false);
          navigate(`/creatives?roas_op=${match[1] === ">" ? "gte" : "lte"}&roas_val=${match[2]}`);
        } else {
          toast.error("Usage: /roas >2.0 or /roas <1.0");
        }
      },
    });
    return cmds;
  ...
  // Filter slash commands based on typed input
  ...
  // ── Flat item lists for keyboard nav ──
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter") {
        ...
        } else if (isSearchMode && flatSearchResults[activeIndex]) {
          handleSelectResult(flatSearchResults[activeIndex]);
        } else if (isDefaultMode && defaultItems[activeIndex]) {
            ...
            {isCommandMode && (
              <>
                ...
                {filteredCommands.length === 0 && (
                  <EmptyState text={`No command matching "${query}"`} />
                )}
            ...
            {isDefaultMode && (
              <>
                ...
                {recentItems.length === 0 && quickActions.length === 0 && (
                  <EmptyState text="Start typing to search, or / for commands" />
                )}
              </>
            )}
            ...
            {isSearchMode && (
              <>
                ...
                {!loading && flatSearchResults.length === 0 && (
                  <EmptyState text={`No results for "${debouncedQuery}"`} />
                )}
                      ...
                      {group.total > 3 && (
                        <button
                          onClick={() => {
                            ...
                            navigate(`${navMap[group.category] || "/creatives"}?q=${encodeURIComponent(debouncedQuery)}`);
                          }}
...
function CommandRow({ index, active, icon, label, subtitle, onSelect, onHover }: {
