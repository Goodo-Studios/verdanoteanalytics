...
async function downloadAndUpload(
  ...
  try {
    ...
    const filePath = `${userId}/import-${crypto.randomUUID()}.${ext}`;

    const { error } = await supabaseAdmin.storage
      .from("ad-media")
      .upload(filePath, new Uint8Array(arrayBuf), { contentType, upsert: false });
    ...
    console.error("Download/upload failed:", (e as Error).message);
    return null;
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!Array.isArray(ads) || ads.length === 0) {
      return json({ success: false, error: "No ads provided" }, 400);
    }
    ...
    for (let i = 0; i < ads.length; i += BATCH) {
      ...
      for (const ad of batch) {
        try {
          ...
          if (!sourceUrl) {
            failed++;
            errors.push(`Ad missing source_url at index ${i}`);
            continue;
          }
          ...
          const { data: saved, error: insertErr } = await supabase
            ...
            .insert({
              ...
              thumbnail_url: storedThumb || thumbUrl,
          ...
          if (insertErr || !saved) {
            failed++;
            errors.push(`Insert failed: ${insertErr?.message || "unknown"}`);
            continue;
          }
          ...
          for (const bName of adBoards) {
            ...
            if (boardId) {
              await supabase.from("ad_library_board_ads").insert({
                board_id: boardId,
                ad_id: saved.id,
          ...
          for (const tName of adTags) {
            ...
            if (tagId) {
              await supabase.from("ad_library_ad_tags").insert({
                ad_id: saved.id,
                tag_id: tagId,
              } as any).select().maybeSingle();
            }
    ...
    return json({
      ...
      failed,
      total: ads.length,
      boards_created: Object.keys(boardMap).length,
      tags_created: Object.keys(tagMap).length,
