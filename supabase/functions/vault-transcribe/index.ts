...
Deno.serve(async (req) => {
  ...
  if (!groqKey && !deepgramKey) {
    return json({ error: "No transcription provider configured (set DEEPGRAM_API_KEY or GROQ_API_KEY)" }, 500);
  }
  ...
  try {
    ...
    if (item.file_path) {
      ...
      // No media on this item (e.g. metadata-only scrape). Mark ready — the
      // thumbnail and creator handle are already saved.
    ...
    // Write a non-blank transcript and chain to vault-analyze.
